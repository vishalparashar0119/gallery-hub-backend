...
export const isAdmin = async (req, res, next) => {
      if (!req.cookies.token) return res.status(401).json({ success: false, message: "please login to  access" });
      ...
      try {
            ...
            if (!admin) return res.status(401).json({ success: false, message: "please login to  access" });
